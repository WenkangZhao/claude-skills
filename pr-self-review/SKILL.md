...
"写错的那一行"，而是下面七件事。自审就是把这七件事逐一做成动作。
...
| 把自己写过的 diff / 注释 / 印章反复读三遍 | 去打开**读它的那段代码**、查同 target 还有谁在改、找全部测试 | 要另起炉灶的检查一条都没做，却判了 0 findings |
...
### 6.5 只做"要另起炉灶"的那几项（自审独有的盲区）

前面六步审别人也做。**这一步只在自审时需要**，因为它对付的不是"哪里难"，而是"哪里顺手"。

自审会把**你已经铺好路的东西**反复走：自己写过的 diff、自己写的注释、自己刚生成的产物、
自己算的那个哈希。成本几乎为零，而且每走一遍都增加一点"我很仔细"的感觉。真正漏掉的，
是那些**要新开一条线**才能查的：读它的那段代码、同 target 的其它 PR、你不知道存在的测试文件。

所以自审结束前，把这三条当清单过一遍——它们的共同点是**都不在你已经打开的文件里**：

1. **凡是写下"X 驱动 / 决定 / 被 Z 读取"这类机制句，就去打开 Z。** 然后用 **Z 自己的逻辑**
   跑新旧两版，比较**它产出的东西**，而不是比较你改的那个文件。两者常常问的不是同一个问题：
   一份 `surface.json` 的 diff 只有"加了几行"，而读它的 `_read_non_revertible` 是按**裸方法名**
   建键、同名取更强级别的——真正该问的是"由它生成的那张地图变了什么、有没有重名把已有条目
   悄悄改写"。文件 diff 答不了这个问题。
2. **开 PR 之前**查同 target 还有哪些活跃 PR（`review-standard` 维度 1 的最后一句）。
   审别人时我们都查，审自己时会忘——而两个人同时重发同一份生成物是最典型的撞车。
3. **找全部读到你改动的测试，不是你知道的那一个。**
   `grep -rln "<你改的文件名/符号>" tests/` —— 只跑"我记得有这么个测试"，等于让自己挑考题。
   最该跑的往往是你刚刚在脑子里替它判了"应该没事"的那个。

**元规则：0 findings 是信号，不是结论。** 自审判出零条时，先回答一句：
*我刚做的检查里，有几条是需要另起炉灶的？* 答案是 0，那这次自审只是把做过的事又做了一遍。

> **出事经过**：一次语料重发（58971）。印章、37 行删除逐条溯源、内部名泄漏扫描、行尾归一化——
> 查得很细，判 0 findings。三项漏的全是要另起炉灶的：没打开读 `surface.json` 的代码就在 PR 里
> 写了"它驱动审批地图"；没查同 target 其它 PR；只跑了自己知道的那个语料测试，而实际有**三个**，
> 漏掉的 `test_corpus_encoding.py` 恰恰是查"UTF-8 经代码页往返"的——那种损坏**留下的是合法
> UTF-8**，所以"能正常 decode"什么都证明不了，而我正是靠"我对行尾推理过了，编码应该没事"
> 把它跳过去的。三项补做后结论没变，但在那之前它是**断言**，不是**挣来的**。

...
定级、校准、不该提什么、以及十条判断规则在那里,**和审别人 PR 用的是同一把尺子**。更实际的一点:
ADO 上那个自动评审 agent 判我们 PR 时读的就是那套标准,所以在这里按它走一遍 ≈ **预判它会提什么**。
本文件是流程,不重复那份标准。
...
档位定义与排除清单以 `pr-review/REVIEW.md` 为准——自审和评审用同一把尺子，评审那边才不会
把同一条重新定级。每条带：证据（file:line，引用代码）、失败场景（具体输入 → 具体错误
输出）、修法。写不出失败场景的不是发现。表里没有条目再推。
...
7. **不可争议的评论改完直接 resolve，有取舍的留给评审关。** 不可争议 = 评审给出了明确修法而且照做了：
   措辞、大括号、注释/文档纠正、把一处读取提到循环外、评审指定的一条测试。这些回帖后用
   `scripts/ado_pr.py resolve <pr> <thread>...` 关掉（ADO 的 "Resolved" 就是 status `fixed`），
   免得评审再点一遍。**留 active 的**：没按建议做的、只做了一半的（"做了 A，B 需要真工程"）、
   MAJOR 里选了三个选项之一的、以及总结线程——那些需要评审再判断一次，替他关等于替他表态。
...
- **写操作之后的回读不能否定写操作**：promote 先覆盖了向导槽位，再 `Describe(OpenConfig(...))` 回读；回读打开的是工程里
  *所有* 配置，任何一个坏掉的 `.spe` 都会让它抛出——调用方被告知 promote 失败，槽位却已经被覆盖。凡是不可逆写之后的读，
  要么 catch 后返回已知部分，要么重抛时消息以"已经写成了"开头（58683 / 512985）。
- **"复制规则的测试"不算钉住**：测试自己把三个上限拼一遍再断言，等于重述规则；把调用点还原成裸调用，测试照样绿。
  要让上限在一个方法里汇合、测试调那个方法，或者走真正的调用点（58682 / 512933）。
- **同一个 k-means 有三个调用点时只封一个**：修了度量那一路，分组器那两路还在同一条成本曲线上（640 核实测 402 s）。
  改一个共享 helper 的成本时，grep 全部调用方并逐个说明为什么不改（58682 / 512932）。
...
- `pr-review`：同一套眼光用在别人的 PR 上，含 ADO 拉取/回帖脚本；它的 `REVIEW.md` 是两边
  共用的定级与评论标准。

...
五层像体检：抽血（秒级单测）→ 拍片（不开机的静态合同检查）→ 试装（零件能不能装进真机）
→ 上机路试（真应用真工程跑一圈）→ **让真司机开一趟**（模型读了并照做）。越往下越慢越真。
**别越级**（改了脚本体只跑 Python 单测等于没测），**也别跳级**（为一个参数拼写错误开应用是浪费半小时）。

**第 5 层不是可选的收尾,它是某一类改动唯一的证明**：凡是**模型会读**的东西——语料、cookbook、
工具描述、拒绝文案、skill 文本——前四层全绿也只证明"那句话存在",不证明它起作用。
...
| 语料 / cookbook / XML 注释 | `local_code_engine/ask.py`；要数字才 `eval/loop.py` | **5** |
| 工具描述 / 拒绝文案 / skill 文本 | 同上 + A/B（见第 5 层） | **5** |
...
## 第 5 层 · 模型读了并照做（唯一能证明"模型会读的东西"的那一层）

> **永远。** 一个模型会读的改动，在**一个模型读了它并且照着做**之前都是未验证的。
> **我们自己读一遍不算证据。** 一个字符串断言只证明那句话存在，不证明它起作用。

**为什么**：一次文档改写看起来修好了"生成脚本里多出一步"的问题。A/B 跑完**两臂都是 null**，
一下子暴露了两件事：改写没被证明，**而且根因本来就判错了**——那个词是真实的产品词汇，不是凭空造的
字面量。没有这一层，它会作为一个"修复"发出去，修的却是它根本没造成的问题。

### ⚠️ 对照臂陷阱（第一个要查、也最容易跳过的）

> **永远。** 如果**对照臂**（旧状态）**复现不出那个缺陷**，这个实验就没有分辨力，修复仍然未被证明。
> **说出来，不要报成通过。**

容易跳过是因为：处理臂表现正确，**它自己看起来就像成功**。

### 首选：驱动真的助手端到端

能用真 Sidekick 就用（第 4 层 C 的 `local_code_engine/ask.py --run` 或 `serve.py` 挂到面板）。

### 退路：真的用不了时，用便宜的 subagent 当替身

这是**用户认可的方法，不是临时凑合**。用**全新的** general-purpose subagent、便宜模型。
**绝不用 fork** —— fork 继承了本会话、已经知道答案，实验直接作废。

站得住的做法：

1. **上下文从真产物里抽**，让 agent 看到的是真消费方看到的东西，不是转述。
2. **两臂只差被测的那一处改动，并且断言它**——数差异行数，不符就失败。**带附带差异的 A/B 什么都测不出。**
3. **给 agent 用户的原话**和真实会话状态，加上它必须遵守的沙箱规则。
   **任何暗示缺陷的东西都不给**：不提被测的那个行为。
4. **每个 agent 把输出写进一个具名文件**，回复只给一个 token。**机械地给文件打分是确定性的，
   给散文报告打分不是。**
5. **每臂至少 3 次**，在**同一条消息里**发出去以便并发。
6. **机械打分**：输出里有没有那个被测的调用。

> 和我们的记忆冲突要注意：`feedback_no_parallel_agents` 说不要并发扇出。**这里是例外且是刻意的**
> ——替身 A/B 里，几个便宜的替身**同时跑本身就是那台仪器**，而且是便宜模型、输出写文件、不再开子
> agent。不确定就先问，别默默烧额度。

### 老实读结果

- **对照臂 null = 不确定，不是修好了。** 见上面那条。
- **替身不是真模型。** 便宜替身给出 null，可能是措辞没问题，也可能只是替身不像线上模型那样发挥。
- **你改的那份产物不是模型唯一的输入。** 助手还会从一个仓库外、grep 不到的知识库里检索时，
  "这个短语在别处不出现"只是关于**代码语料**的陈述，**不能泛化成"我们的文档就是源头"**。
- **一个听话的替身，测不了一个真模型不听的提示词。** 有一次替身在**旧**措辞下三次全选对了工具，
  而线上模型五轮才走到一次。**又是 null 对照。** 那次缺陷的证据是**服务端自己的事件日志**，
  而不依赖模型配合的修复是**代码**——一道能力门，有单测、变异证明过。
- **正结果是真证据。** 报比例、报两臂、报用的模型。**绝不只报结论。**

### 路由是问题时，读日志，并把强制放进代码

任何"这个请求走了哪条路"的问题，**读服务端事件日志**：它是直接证据，聊天记录只是推断。
而答案重要时，**把强制放进代码而不是措辞里**。**对提示词措辞做 A/B 是手上最弱的仪器。**

### 每个门都要走

对"助手会做什么"的改动，要通过**每一个**能到达它的入口验证，不是顺手那个。入口之间的路由经常
不确定，所以**每个样本都记录走了哪条路**，至少取三个，**路由没走到的那次既不算支持也不算反对**。

### 重置前置条件，从系统读结果

样本之间用产品自己的自动化接口重置状态。**事后那次读取——查询真实对象——才是结果的证明。
聊天文字永远不是。**
...
- 第 4 层 `sdk-test` 绿：真应用上跑了且 oracle 同意。这是唯一能写"验证过"的信号。
  **但对模型会读的东西，它只证明脚本跑得通，不证明模型会照那句话做。**
- 第 5 层：一个模型读了并照做了，**而且对照臂复现出了缺陷**。**对照臂 null = 不确定，不是修好了。**
...
- `fix-design`：**"修复是代码，否则它不是修复"**——第 5 层反复得出的结论是，住在一句话里的修复会
  回来，住在代码里的守得住。改模型会读的东西之前先读它。
